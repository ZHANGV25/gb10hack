    ...
    <div className="flex min-h-full">
      <aside className="sticky top-0 hidden h-screen w-[236px] shrink-0 flex-col border-r border-hairline bg-surface lg:flex">
        <div className="flex items-center gap-2.5 px-4 py-4">
          ...
          <span className="leading-tight">
            <span className="block text-[14px] font-semibold">Nordhafen Bank</span>
          ...
          {NAV.map((item) => {
            ...
            return (
              <Link
                ...
                className={`group flex items-center gap-2.5 rounded-md px-2.5 py-1.5 text-[14px] transition-colors ${
                  active
              ...
              >
                ...
                {item.badge === "gaps" && open > 0 ? (
                  <span className="rounded-full bg-foreground px-1.5 py-px font-mono text-[11px] leading-4 text-background">
                    {open}
                  </span>
                ) : null}
              </Link>
      ...
      <div className="min-w-0 flex-1">
        <header className="sticky top-0 z-20 flex h-12 items-center gap-4 border-b border-hairline bg-background/85 px-5 backdrop-blur lg:px-8">
          <Link href="/" className="text-[14px] font-semibold lg:hidden">
            Nordhafen · ICT register
          </Link>
          <nav className="flex items-center gap-3 text-[14px] text-muted-foreground lg:hidden">
            {NAV.map((item) => (
              <Link
                key={item.href}
                href={item.href}
          ...
          <p className="ml-auto hidden items-center gap-2 text-[12px] text-muted-foreground lg:flex">
            <span>Analyst workspace</span>
            <span className="text-hairline">·</span>
            <span>M. Halvorsen, Third-Party Risk</span>
          </p>
          <span className="rounded-full border border-hairline px-2 py-0.5 text-[11px] tracking-wide text-muted-foreground">
            Confidential
          </span>
        </header>
        {children}
...
export function Page({
  ...
  return (
    <main className="mx-auto max-w-[1260px] px-5 py-7 pb-20 lg:px-8">
      <div className="flex flex-wrap items-start justify-between gap-4">
        <div className="min-w-0">
          <h1 className="text-[21px] font-semibold">{title}</h1>
          {lede ? (
            <p className="mt-1 max-w-2xl text-[14px] leading-5 text-muted-foreground">
              {lede}
            </p>
          ) : null}
        </div>
