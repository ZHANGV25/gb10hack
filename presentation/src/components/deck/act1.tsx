...
export function SceneFilm() {
  ...
  return (
    <div className="absolute inset-0">
      <RegMarks />
      <div className="absolute inset-x-0 bottom-[11vh] top-[5vh] z-10 flex items-center justify-center px-[3.5vmin]">
        <video
          ref={videoRef}
          src="/film.mp4"
          poster="/shots/register.png"
          preload="auto"
          onEnded={() => setEnded(true)}
          className="max-h-full max-w-full border border-ink/30"
          style={{ aspectRatio: "16 / 9", filter: started ? "none" : "grayscale(0.2) opacity(0.85)" }}
        />
      </div>
      {!started && (
        <SlideLabel delay={0.6}>
          The film — recorded against the live desk · enter rolls it
        </SlideLabel>
      )}
      {ended && (
        <SlideLabel delay={0.2}>The film has ended — advance when ready</SlideLabel>
      )}
    </div>
  );
}
...
/* ── Slide 0 — cover ─────────────────────────────────────────────────────
   The frame on screen while the room settles. Wordmark, one rule, no ask. */
    ...
    <div className="absolute inset-0">
      ...
      <div className="absolute inset-0 z-10 flex flex-col items-center justify-center gap-[2.4vmin]">
        <motion.div
          ...
          style={{ fontFamily: "var(--font-display)", fontSize: "13vh" }}
        >
          ExitPlan
        </motion.div>
        <motion.div
          ...
          className="h-[0.35vmin] w-[26vmin] origin-left bg-tan"
        />
        <motion.div
          initial={{ opacity: 0 }}
          animate={{ opacity: 1 }}
          transition={{ delay: 1.3, duration: 0.6 }}
          className="mark-label text-[1.6vmin] text-g600"
        >
        ...
        <motion.div
          ...
          className="mark-label text-[1.3vmin] text-g600"
        >
          EuroMaxing · Dell Pro Max GB10
        </motion.div>
      </div>
    </div>
  );
